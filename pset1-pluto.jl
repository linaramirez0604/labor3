...
We can define the problem as a profit maximization problem. Specifically, consider

```math
\begin{gather*}
	\max_{c,h} \frac{1}{\lambda} u(c,h) - c + wh 
\end{gather*}
```
...
```math
\begin{align*}
	[c]: & \quad (c^\sigma + (1-h)^\sigma)^{\frac{1}{\sigma}-1} c^{\sigma-1} = \lambda \\
	[h]:&  \quad (c^\sigma + (1-h)^\sigma)^{\frac{1}{\sigma}-1} (1-h)^{\sigma-1} = \lambda w
\end{align*}
```
...
```math
\begin{gather*}
	  c = w^{\frac{1}{1-\sigma}}(1-h)
\end{gather*}
```

Combining this expression with the first FOC gives

```math
\begin{gather*}
    \frac{(w^{\frac{\sigma}{\sigma-1}}+1)^\frac{1-\sigma}{\sigma}}{w} = \lambda
\end{gather*}
```
...
md"""
**Co-movement of Hours and Wages**

We are back to the case where 

```math 
	u(c,h) = c^\alpha(1-h)^\gamma
```

In class, we saw that the Frisch elasticity of hours with respect to wages is given by

```math 
	\frac{\partial h^F}{\partial w} = \frac{-1}{D}u_{cc}\lambda
```

where 

```math 
	D = u_{cc}u_{hh}-u_{ch}^2 
```

and 

```math 
	\lambda = \left(\frac{b_0 + \sum_t \beta^t w_t}{\kappa (1+\frac{\alpha}{\gamma})\sum\beta^tw_t^{\frac{\gamma}{\gamma+\alpha-1}}} \right)^{\alpha+\gamma-1}
```

We know that $\lambda$ is the marginal value of wealth, so $\lambda>0$. This implies that $\alpha>-\gamma$. We want to have $\frac{\partial h^F}{\partial w}>0$. This implies that $D$ and $u_{cc}$ must be of opposite sign. It makes economic sense to have utility concave in consumption, $u_{cc}<0$. With our functional form, we have that 

```math 
	u_{cc} = (\alpha-1)\alpha c^{\alpha-2}(1-h)^\gamma 
```

This implies that $\alpha<1$ to get $u_{cc}<0$. Now, we want $D>0$. Using our functional form once again, we have that 

```math 
	D = (\alpha-1)\alpha(\gamma-1)\gamma c^{2(\alpha-1)}(1-h)^{2(\gamma-1)} - (\gamma\alpha c^{\alpha-1}(1-h)^{\gamma-1})^2
```

Simplifying, we obtain
```math 
	D = \alpha\gamma c^{2(\alpha-1)}(1-h)^{2(\gamma-1)}((\alpha-1)(\gamma-1)-\gamma\alpha)
```

Notice that $\alpha\gamma c^{2(\alpha-1)}(1-h)^{2(\gamma-1)}>0$ because $\alpha>0, \gamma>0$. So in order to get $D>0$ we just need 

```math
	(\alpha-1)(\gamma-1)>\gamma\alpha
```
...
which, as seen in class, can be written 
...
Following the conclusions of the prior parts, we set $0 < \alpha < 1$ and $0 < \gamma < 1$ such that $\alpha + \gamma < 1$. Here, we choose $\alpha = 0.70$ and $\gamma = 0.15$. We set the starting asset equal to 8. We may adjust these parameters below. This may also be adjusted. There is no borrowing constraint. 
"""

# ╔═╡ 93f134fd-e7e9-4aaf-a5e1-1111aac196ef
md"""
- ``\alpha``: $(@bind alpha Slider(LinRange(0,1,50), default=0.7)) 
- ``\gamma``: $(@bind gamma Slider(LinRange(0,1,50), default=0.15)) 
- ``b_0``: $(@bind b0 Slider(LinRange(0,10,50), default=8))
"""
...
begin
	...
	# Frisch
	h_f(w,lambda) = 1 - kappa * w^( (1-alpha)/(alpha+gamma-1) ) * lambda^(1/(alpha+gamma-1))
...
begin
	...
	lambda_t(w_t,t_t) = (b0 + sum(beta.^t_t .* w_t) )^rho / sum(  kappa * (1+alpha/gamma) * beta.^t_t .* w_t.^(gamma/rho))^(rho)
end
...
begin
	...
	ws_t = 0.5 .+ 0.001 * 25^2 .- 0.001 * (ts_t .- 25).^2
	lambda_d1 = lambda_t(ws_t,ts_t)

	ht_1 = h_f.(ws_t,lambda_d1)
	ct_1 = c_f.(ws_t,lambda_d1)

	bt_1 = zeros(Ts)
	bt_1[1] = b0 	
	for t in 2:Ts
		bt_1[t] = beta^-1 * bt_1[t-1] + ws_t[t-1]*ht_1[t-1] - ct_1[t-1]
	end
	
	p1 = plot(ts_t, ws_t, label="wage")
