...
# ╔═╡ 4fdae33a-8d84-444f-ab51-64b8a2b3bfdd
md"""
...
We can define the problem as a profit maximization problem:
```math
...
```math
        [c]:   (c^\sigma + (1-h)^\sigma)^{\frac{1}{\sigma}-1} c^{\sigma-1} = \lambda
```
```math
        [h]: (c^\sigma + (1-h)^\sigma)^{\frac{1}{\sigma}-1} (1-h)^{\sigma-1} = \lambda w
```
Combining the two we get: 
```math
          c = w^{\frac{1}{1-\sigma}}(1-h)
```
Replacing this expression in any of the FOCs we get that
```math
    \frac{(w^{\frac{\sigma}{\sigma-1}}+1)^\frac{1-\sigma}{\sigma}}{w} = \lambda
```
This implies that we can't derive Frish elasticities from this utility function (CES).  

"""



# ╔═╡ 5522fd16-c616-4a33-94b8-1989fbdc6a33
md""" 
**Co-movement of hours and wages**

We are back to the case were 

```math 
	u(c,h) = c^\alpha(1-h)^\gamma
```
In class we got that the Frish elasticity of hours with respect to wages is given by
```math 
...
We know that $\lambda$ is the marginal value of wealth so $\lambda>0$ which implies that $\alpha>-\gamma$. We want to have $\frac{\partial h^F}{\partial w}>0$. This implies that $D$ and $u_{cc}$ must be of opposite sign. It makes economic sense to have marginal decreasing utility to consumption, $u_{cc}<0$. With our functional form we have that 
```math 
...
begin
	...
	h_h(w,u) = 1 -  ( alpha/gamma * u^(-1/alpha) *w)^(-alpha/(alpha+gamma))
	
	# lambda_marshallian
	lambda_m(w,R) = alpha * c_m(w,R)^(alpha-1) * (1 - h_m(w,R))^gamma
